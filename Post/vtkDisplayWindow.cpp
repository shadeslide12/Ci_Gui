...
std::vector<vtkSmartPointer<vtkActor>> vtkDisplayWindow::CreateBladeToBladePlane(double span)
{
    ...
    std::cout << "[B2B vtkDisplayWindow] Creating B2B plane at span = " << span << std::endl;
    
    // Check if node_radius exists
    if(aesReader.node_radius.empty())
    {
        std::cout << "[B2B ERROR] node_radius field not found. Cannot create blade-to-blade plane." << std::endl;
        std::cout << "[B2B ERROR] Make sure you loaded grid.h5 generated by preprocessing!" << std::endl;
        return actors;
    }
    
    std::cout << "[B2B] node_radius size: " << aesReader.node_radius.size() << std::endl;
    ...
    std::cout << "[B2B] Total grid points: " << TotalMeshes->GetNumberOfPoints() << std::endl;
    ...
    std::cout << "[B2B] Contour output points: " << contour->GetOutput()->GetNumberOfPoints() << std::endl;
    std::cout << "[B2B] Contour output cells: " << contour->GetOutput()->GetNumberOfCells() << std::endl;
    
    if (contour->GetOutput()->GetNumberOfPoints() == 0)
    {
        std::cout << "[B2B ERROR] Contour is empty! Span value may be out of range." << std::endl;
        auto radiusArray = TotalMeshes->GetPointData()->GetArray("radius");
        if (radiusArray) {
            double range[2];
            radiusArray->GetRange(range);
            std::cout << "[B2B INFO] Valid radius range: [" << range[0] << ", " << range[1] << "]" << std::endl;
        }
        return actors;
    }
    
    contour->GetOutput()->GetPointData()->SetActiveScalars(Flow[curFlow].name.c_str());
    BladeToBladePlane.emplace_back(contour);
    ...
    std::cout << "[B2B SUCCESS] Created blade-to-blade plane at span = " << span << std::endl;
    std::cout << "[B2B] Actor created and added to list" << std::endl;
    return actors;
}
