...
private:
    ...
    bool scalarBarIsVertical = true;  // 記錄主 ScalarBar 的方向，默認垂直
    ...
    std::map<int, std::vector<vtkSmartPointer<vtkActor>>> periodicCopyMeshActorsByZone;
    std::map<int, std::vector<vtkSmartPointer<vtkActor>>> periodicCopyEdgeActorsByZone;
